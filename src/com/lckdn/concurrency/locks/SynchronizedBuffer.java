package com.lckdn.concurrency.locks;
...
public class SynchronizedBuffer {
    ...
    private static void blockingWrite() {
        for (int i = 0; i < 10; i++) {
            try {
                while (!isEmpty) {
                    System.out.println(BLUE + "Writer is trying to access a resource");
    ...
    private static void blockingRead() {
        for (int i = 0; i < 10; i++) {
            try {
                ...
                isEmpty = true;
                System.out.println(GREEN + "Reader reads from buffer: " + readValue);
                canWrite.signal();
            } catch (InterruptedException e) {
                e.printStackTrace();
            } finally {
